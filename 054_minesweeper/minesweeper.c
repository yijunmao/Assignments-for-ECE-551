...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  for (int i = 0; i < h; i++) {
    board->board[i] = malloc(w * sizeof(*(*(board->board))));
  }

  for (int j = 0; j < h; j++) {
    for (int i = 0; i < w; i++) {
      board->board[j][i] = UNKNOWN;
    }
  }
...
int countMines(board_t * b, int x, int y) {
  //WRITE ME!
  int num = 0;
  if (x - 1 >= 0 && IS_MINE(b->board[y][x - 1])) {
    num++;
  }
  if (x + 1 < b->width && IS_MINE(b->board[y][x + 1])) {
    num++;
  }
  if (y - 1 >= 0 && IS_MINE(b->board[y - 1][x])) {
    num++;
  }
  if (y + 1 < b->width && IS_MINE(b->board[y + 1][x])) {
    num++;
  }
  if (x - 1 >= 0 && y - 1 >= 0 && IS_MINE(b->board[y - 1][x - 1])) {
    num++;
  }
  if (x - 1 >= 0 && y + 1 < b->height && IS_MINE(b->board[y + 1][x - 1])) {
    num++;
  }
  if (x + 1 < b->width && y + 1 < b->height && IS_MINE(b->board[y + 1][x + 1])) {
    num++;
  }
  if (x + 1 < b->width && y - 1 >= 0 && IS_MINE(b->board[y - 1][x + 1])) {
    num++;
  }
